...
function sortChapters(bookEl){
  ...
  Array.from(parts.children).slice(2).filter(el=> el.innerText.startsWith("Глава")).sort(function (a, b) { return Number(a.innerText[6]) - Number(b.innerText[6])}).forEach(function (ele) {
    parts.appendChild(ele);
  });

  Array.from(parts.children).slice(2).filter(el=> el.innerText.startsWith("Приложение")).sort((a, b)=> a.innerText.localeCompare(b.innerText)).forEach(function (ele) {
    parts.appendChild(ele);
  });
}
